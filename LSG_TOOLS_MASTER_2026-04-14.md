...
- Deal detail modal with key metrics, screener, and "Open in One Pager" button
- **Inline deal editing:** "Edit Fields" button in modal toggles editable form; saves directly to Supabase via PATCH; covers deal info, property metrics, and returns/pricing fields
...
- `toggleEditPanel()` — toggles inline edit form inside deal modal
- `saveEdits()` — PATCHes top-level columns + nested `raw_data` fields to Supabase; updates local cache and re-renders pipeline table
...
**lsg_ingest.html — inline deal editing:**
- Added "Edit Fields" toggle button in deal detail modal footer
- `toggleEditPanel()` — renders editable form appended to modal body; groups fields into Deal Info, Property Metrics, Returns & Pricing; toggle cancels without saving
- `saveEdits()` — reads all edit inputs; PATCHes top-level columns (`deal_name`, `address`, `asset_type`, `status`, `source_broker`, `purchase_price`, `going_in_cap`, `irr_levered_5`, `moic_5`, `coc_1`) and nested `raw_data` fields (`property.*`, `returns.*`, `sources_uses.*`, `capital_sources.*`, `market.name`, `cash_flows.noi[0]`) in one PATCH call; updates `allDeals` local cache; refreshes pipeline table; re-opens modal to reflect changes; shows inline save status
...
- [x] **Inline deal editing in pipeline modal** — edit missing fields directly from pipeline tab; PATCH to Supabase ✓ April 14
- [ ] **Test full workflow end-to-end** — ingest OM → pipeline tab → edit fields → verify Supabase → open in one-pager → DB sync
